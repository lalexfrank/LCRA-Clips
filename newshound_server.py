  ...
  <aside>
    <div>
      ...
      <div class="cat-builder">
        ...
        <button id="add-cat-btn">+ Add</button>
      </div>
  ...
  categories.forEach(cat => {
    ...
    card.innerHTML = `
      <div class="cat-card-hdr">
        ...
        <button class="cat-rm" data-rm="${cat.id}">×</button>
      </div>
      <div class="kw-row">
        <input type="text" placeholder="Add keyword…" data-inp="${cat.id}">
        <button data-add="${cat.id}">+</button>
      </div>
...
<table style="width:100%;max-width:680px;margin:0 auto;background:#ffffff" cellspacing="0" cellpadding="0" border="0">
  <tbody>
    ...
    <tr><td style="padding:16px 18px;border-top:1px solid #cccccc;">
      <p style="font-family:Helvetica;font-size:11px;color:#aaaaaa;margin:0;text-align:center;">Generated by NewsHound · ${today}</p>
    </td></tr>
  </tbody>
</table></body></html>`;
